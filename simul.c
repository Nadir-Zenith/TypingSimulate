...
{
  int opt;
  int chdelay = 40, spdelay = 40, nldelay = 30;
	char str[] = "This line says it all";
  ...
  for (opt = optind; opt < argc; opt++)
    ...
    {
      echodelay (chdelay, spdelay, nldelay, str);
      putchar (' ');
    }
...
{
	int pos = 0;
	//add a delay between printing each character in the string, depending on the character
	do
	{
		switch(string[pos])
		{
